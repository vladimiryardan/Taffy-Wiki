...
Your API should exist as a separate application context from other applications. As such, it should exist in a separate folder and have its own Application.cfc. At a minimum, your APIs Application.cfc needs the following:
...
h1. API Resource CFCs

Each resource in your API (eg. Artists, Art) should be defined as its own CFC. *Collections and Members should be separate CFCs* (eg. @artistCollection.cfc@ and @artistMember.cfc@). Don't worry too much about the file names, they won't be exposed anywhere. In addition, if you like, your API CFCs may be outside the web-root, as long as they are accessible via CF mappings (application-specific mappings are acceptable).
...
* Each CFC should implement _up to_ 4 methods: *get*, *post*, *put*, and *delete*. As you may have guessed, these map directly to the HTTP verb used by the api consumer. 

* If the consumer uses the PUT verb, it runs the PUT method in the corresponding CFC. 
...
* Tokens from @taffy_uri@, defined as {token} (see example above) will be extracted from the URI and passed by name to the requested method. In addition, all query string parameters (except those defined for debugging and reloading) will also be included in the argument collection sent to the method. <br/>For example: @GET /artist/44/art?city=Philadelphia@ will result in the GET method being called, with the arguments: @{ artistId: 44, city: "Philadelphia" }@.

* The @representationOf@ method is a special method that creates a new object that is capable of serializing your data into the appropriate format. A generic class capable of serializing anything into JSON is included with the framework and used by default. To use a custom object, pass the dot-notation CFC path as the 2nd argument to the @representationOf@ method. (It is also planned to implement a global default representation object override, but that is not yet available.) See [[Using a Custom Representation Class]] for more details.