...
h3(#addURI). addURI(string cfcPath)
...
h3(#applicationHook). applicationHook()
...
h3(#defaultMime). defaultMime(string DefaultMimeType)
...
h3.(#registerMimeType) registerMimeType(string extension, string mimeType)
...
h3(#setDebugKey). setDebugKey(string keyName)
...
h3(#setReloadKey). setReloadKey(string keyName)
...
h3(#setReloadPassword). setReloadPassword(string Password)
...
h3(#representationOf). representationOf(any data, [string customRepresentationClass])
...
h3(#withStatus). withStatus(numeric statusCode)
...
This special method may _*only*_ be used in conjunction with *noData* or *representationOf*. It sets the HTTP Status Code of the return.
